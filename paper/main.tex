...
% Loosen the LaTeX float-placement defaults so tables and figures can sit
% near the text that references them rather than getting deferred to the
% next available page-top.
\renewcommand{\topfraction}{0.92}      % up to 92% of a page can be floats at top
\renewcommand{\bottomfraction}{0.6}    % up to 60% at bottom
\renewcommand{\textfraction}{0.08}     % only 8% text required on a float page
...
\begin{table}[!htbp]
  \centering
  \caption{Stage~2 Random Forest Hyperparameter Search Space}
  \label{tab:hp}
  \footnotesize
...
\begin{figure*}[!htbp]
  ...
  \caption{Field-deployment architecture.}
  \label{fig:deploy}
\end{figure*}
...
server that exposes three input paths. A JSON \texttt{POST
/api/predict} endpoint serves one-shot HTTP queries, a SocketIO
\texttt{predict} event serves low-latency clients, and a TCP listener
on port 5555 accepts newline-delimited JSON frames of the form
\texttt{\{label, signal:[v\textsubscript{0}, \ldots,
v\textsubscript{99}], window\}}. Each prediction received over the
TCP path is rebroadcast as a SocketIO \texttt{pqd\_result} event to
every connected dashboard. The included MATLAB sender and the Python
sender both publish through this path; the Python sender additionally
generates realistic synthetic waveforms in accordance with the
IEEE~1159 disturbance definitions.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\columnwidth]{figures/dashboard.png}
  \caption{Real-time browser dashboard.}
  \label{fig:dashboard}
\end{figure}
...
\begin{table}[!htbp]
  \centering
  \caption{Classical Classifier Comparison on the 36-Feature Vector}
  \label{tab:modelcmp}
  \footnotesize
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{|l|c|c|c|}
    ...
    \hline
    \textbf{Macro avg}     & \textbf{98.85} & \textbf{98.85} & \textbf{98.85} \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  \centering
  \caption{Per-Stage Inference Latency (ms)}
  \label{tab:latency}
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \renewcommand{\arraystretch}{1.2}
