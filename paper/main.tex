...
\begin{equation}
  ...
  \label{eq:kurt}
\end{equation}
This is the Pearson form of kurtosis, for which a clean unit-amplitude
sinusoid yields $K \approx 1.5$. The detector trips if $K > 2.5$
...
\begin{table}[t]
  \centering
  \caption{Stage 2 Random Forest hyperparameter grid search. The
           deployed configuration is retained because the grid-search
           optimum exceeds it by less than the test-set noise floor.}
  \label{tab:hp}
  \begin{tabular}{lll}
    \toprule
    Hyperparameter & Search space & Deployed value \\
    \midrule
...
\begin{itemize}
  ...
  \item \textbf{Stage~2 CNN forward pass.} The forward pass is
        dominated by the three Conv1D layers. The measured mean
        latency is approximately 40~ms.
